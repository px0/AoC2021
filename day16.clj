...
(defmulti parse-subpackages  (fn [_ p]
                               (not= (:type p) ::literal)))

(defmethod parse-subpackages false
  [b p]
  (loop [num []]
...
(defmethod parse-subpackages true
  [b p]
...
(defmethod eval ::literal [p]
  (:value p))

(defmethod eval ::sum [p]
  (reduce + (map eval (:children p))))

(defmethod eval ::product [p]
  (apply * (map eval (:children p))))

(defmethod eval ::min [p]
  (apply min (map eval (:children p))))

(defmethod eval ::max [p]
  (apply max (map eval (:children p))) )

(defmethod eval ::gt [p]
  (let [[one two] (map eval (:children p))]
    (if (> one two) 1 0)))
...
(test/deftest unittests
  ...
  (test/is (= (eval {:version 6, :type-val 0, :type :day16/sum, :children [{:version 6, :type-val 4, :type :day16/literal, :value 1} {:version 2, :type-val 4, :type :day16/literal, :value 2}]})
              3))

  (test/is (= (eval {:version 6, :type-val 3, :type :day16/max, :children [{:version 0, :type-val 4, :type :day16/literal, :value 7} {:version 5, :type-val 4, :type :day16/literal, :value 8} {:version 0, :type-val 4, :type :day16/literal, :value 9}]})
              9)))
